...
async def yt_service_run(url: str, user_id: int, podcast: bool = False):
    yt = YouTubeService(url=url, podcast=podcast)
    try:
...
def yt_task(url: str, user_id: int, podcast: bool = False):
